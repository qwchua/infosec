...
app.get('/api/getbookmarks/:userid', (req, res)=> {
    const sql = `SELECT * FROM CRUDDatabase.Bookmark b, CRUDDatabase.Product p WHERE p.product_id = b.product_id AND b.user_id = ${req.params.userid} ORDER BY date DESC`
...
app.get('/api/sales/:userid', (req, res)=> {
    const sql = `SELECT o.order_id, o.order_date, o.buyer_name, o.buyer_address, o.buyer_postal, o.buyer_phone, p.product_name, p.price FROM CRUDDatabase.OrderHistory o, CRUDDatabase.Product p WHERE o.product_id = p.product_id AND o.seller_id= ${req.params.userid} ORDER BY order_id DESC`
    let query = db.query(sql,(err,result)=>{
        if(err) throw err;
        res.send(result);
    })
...
//get user average score round to 1 decimal place
app.get('/api/averagereviewscore/:userid', (req, res)=> {
    const sql = `SELECT ROUND(AVG(rating),1) as 'averageScore' FROM CRUDDatabase.OrderHistory o, CRUDDatabase.Account a WHERE o.seller_id = a.user_id AND a.user_id = ${req.params.userid}`
    let query = db.query(sql,(err,result)=>{
        if(err) throw err;
        res.send(result);
    })
...
app.get('/api/purchases/:userid', (req, res)=> {
    const sql = `SELECT o.order_id, o.order_date,a.username, p.product_name, p.price FROM CRUDDatabase.OrderHistory o, CRUDDatabase.Product p, CRUDDatabase.Account a WHERE o.product_id = p.product_id AND p.seller_id = a.user_id AND o.buyer_id= ${req.params.userid} ORDER BY order_id DESC`
    let query = db.query(sql,(err,result)=>{
        if(err) throw err;
        res.send(result);
    })
...
app.get('/api/search2/:searchquery', (req, res)=> {
    const sqlSelect = `SELECT product_name, price, stock FROM CRUDDatabase.products WHERE product_name LIKE "%${req.params.searchquery}%"`
